...
async function slideNext() {
  ...
  setTimeout(() => {
    ...
    Array.from(track.children).forEach((card, i) => {
      ...
      if (img) {
        img.src = i === CENTER_INDEX ? img.dataset.animated : img.dataset.static;
      }
    });
    isSliding = false;
  }, 500);
...
async function searchPokemon(query) {
  ...
  const matches = allPokemonNames.filter((name) => name.includes(query.toLowerCase()));
...
async function getEvoChainLength(species) {
  ...
  let count = 0;
  let node = data.chain;
  while (node) { count++; node = node.evolves_to?.[0] ?? null; }
  return count;
}
...
async function applyFilters() {
  ...
  const letters = [...document.querySelectorAll("#filter-letters .filter-pill.active")].map((p) => p.dataset.value.toLowerCase());
  ...
  let finalNames = idSets.reduce((a, b) => new Set([...a].filter((x) => b.has(x))));

  const pokemons = (await Promise.all([...finalNames].map(async (name) => {
    const res = await fetch(`${BASE_URL}/pokemon/${name}`);
    if (!res.ok) return null;
    return res.json();
  }))).filter(Boolean);

  let filtered = letters.length ? pokemons.filter((p) => letters.includes(p.name[0])) : pokemons;
  ...
  if (pokemonId) {
    (async () => {
      ...
      if (prevEvo) {
        ...
        document.getElementById("detail-evolves-from").addEventListener("click", () => {
      ...
      if (currentEvo?.evolvesTo?.length) {
        ...
        document.getElementById("detail-evolves-to").addEventListener("click", () => {
